...
$(function () {
    layui.use(['table', 'form'], function () {
        ...
        //监听提交
        form.on('submit(adminUserSubmit)', function (data) {
            // TODO 校验
            formSubmit(data);
            return false;
        });
    ...
    if (null != id) {
        if (currentUser == id) {
            ...
            layer.confirm('您确定要删除' + name + '用户吗？', {
                ...
                $.post("/adminUser/updateAdminUserStatus", {"id": id, "status": 0}, function (data) {
                    if (data.code == 1) {
                        layer.alert(data.message, function () {
                            layer.closeAll();
                            load(obj);
...
function recoverUser(obj, id) {
    if (null != id) {
        layer.confirm('您确定要恢复吗？', {
            ...
            $.post("/adminUser/updateAdminUserStatus", {"id": id, "status": 1}, function (data) {
                if (data.code == 1) {
                    layer.alert(data.message, function () {
                        layer.closeAll();
                        load(obj);
